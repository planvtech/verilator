...
`endif
  ...
  assert property (@(posedge clk) disable iff (cyc < 2) (a and b) |-> (a & b));

  // Boolean 'or': (a or b) true iff a || b
  assert property (@(posedge clk) disable iff (cyc < 2) (a or b) |-> (a | b));

  // 'and' with constant true -- reduces to just 'a'
  assert property (@(posedge clk) disable iff (cyc < 2) (a and 1'b1) |-> a);

  // 'or' with constant false -- reduces to just 'a'
  assert property (@(posedge clk) disable iff (cyc < 2) (a or 1'b0) |-> a);
  ...
  assert property (@(posedge clk) (a & b) |-> (a ##1 1'b1) and(b ##1 1'b1));

  // Sequence 'and': different lengths, end at later branch
  assert property (@(posedge clk) (a & b) |-> (a ##1 1'b1) and(b ##2 1'b1));

  // Sequence 'and': standalone (constant, always passes)
  assert property (@(posedge clk) (1'b1 ##1 1'b1) and(1'b1 ##2 1'b1));
  ...
  assert property (@(posedge clk) (a | b) |-> (a ##1 1'b1) or(b ##1 1'b1));

  // Sequence 'or' where both branches end in registered state (no finalCondp).
  // Exercises guardedLink(termVertexp,...) in buildOrMerge for both sides.
  cover property (@(posedge clk) (a [* 2]) or(b [* 2]));

  // Unequal-end OR / unbounded temporal AND diagnostics live in t_property_nfa_composite_unsup.
  ...
  assert property (@(posedge clk) not (a ##1 b)) $display("negated with pass action");
  ...
  assert property (@(posedge clk) not (a ##1 (b [* 2]))) $display("negated state-end with pass");
