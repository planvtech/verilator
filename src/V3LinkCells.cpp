...
class LinkCellsVisitor final : public VNVisitor {
    ...
    std::unordered_map<AstNodeModule*, std::vector<AstNodeModule*>> ModuleMap;
    std::map<AstNodeModule*, AstCell*> ModuleCellMap;
    // METHODS
    ...
    void visit(AstCell* nodep) override {
        ...
        if (!nodep->modp() || cloned) {
           ...
           // ModuleMap[m_modp] = cellmodp;
            ...
            if (cellmodp) {
                //detect cycleeee
                std::vector<std::vector<AstNodeModule*>> cycles = findAllCycles(ModuleMap);
